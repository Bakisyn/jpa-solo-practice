...
public class YogaSessionServiceTest {
    ...
    class PatchingASession{
        ...
        @Test
        void should_returnSession_when_patchingSession_and_patchWentSuccessfully() throws IOException {
            when(sessionInputChecker.checkNumberFormat(anyString())).thenReturn(session.getId());
            when(yogaSessionRepository.findById(session.getId())).thenReturn(Optional.ofNullable(session));
            ...
            sessionService.patchSession("121",jsonPatch);
        }
        @Test
        void should_throwException400BadRequest_when_patchingSession_and_patchWentUnsuccessfully() throws IOException {
            when(sessionInputChecker.checkNumberFormat(anyString())).thenReturn(session.getId());
            when(yogaSessionRepository.findById(session.getId())).thenReturn(Optional.ofNullable(session));
            when(patcher.patch(any(),eq(session))).thenThrow(new BadRequestApiRequestException("test message"));

            String myPatchInfo = updatePatchInfo.replace("2025-05-22",LocalDate.now().plusDays(1).toString());
            InputStream in = new ByteArrayInputStream(myPatchInfo.getBytes(StandardCharsets.UTF_8));
            jsonPatch = mapper.readValue(in, JsonPatch.class);

            when(patcher.patch(eq(jsonPatch),eq(session))).thenThrow(new BadRequestApiRequestException("test message"));


            Exception exception = assertThrows(BadRequestApiRequestException.class, ()-> sessionService.patchSession("121",jsonPatch));
...
//            when(roomRepository.findSingleRoomByDateAndType(any(),any())).thenReturn(roomTwo);
